...
def start(url, save_path=IMAGE_SAVE_BASEPATH, success_callback=None, fail_callback=None):
    ...
    for topic in topics:
        if topic.has_key("save_path"):
            t = IlluDownloadThread(topic.href, topic.save_path).register_hook(
...
class PixivisionLauncher(threading.Thread):
    def run(self):
        start(self.url, save_path=self.save_path, success_callback=self.success,
              fail_callback=self.fail)

    def __init__(self, url, save_path=IMAGE_SAVE_BASEPATH):
