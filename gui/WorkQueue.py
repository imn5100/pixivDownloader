        ...
        while True:
            try:
                ...
                if task.has_key('id') and task.has_key('path'):
                    illu_file = self.downloader.download_all_by_id(task.get('id'), task.get('path'))
                    if callback:
                        msg = CommonUtils.build_callback_msg(illu_file, id=str(task.get('id')))
                        callback(msg)
                elif task.has_key('path') and task.has_key('url'):
                    illu_file = self.downloader.download_all_by_url(task.get('url'), task.get('path'))
                    if callback:
                    ...
                    illu_file = self.downloader.download_illustration(task, task.get('search_path'),
                    ...
                    if callback and illu_file:
                        callback("{\n%s:%s\nFile:%s\n}\n" % ("search get", task.get('title'), illu_file))
                elif task.has_key('ranking_path'):
                    illu_file = self.downloader.download_by_detail(task, task.get('ranking_path'),
                                                                   p_limit=task.get('p_limit'))
                    if callback and illu_file:
                        callback("{\n%s:%s\nFile:%s\n}\n" % ("ranking get", task.get('title'), illu_file))
